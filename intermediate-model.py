...
fasttext.util.download_model(
    "el", if_exists="ignore"
)  # Download Greek model if not present
...
class TweetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        vecs = [
            self.model[word] if word in self.model else np.zeros(300) for word in tokens
        ]
        padding = [np.zeros(300)] * (self.max_len - len(vecs))
        vecs.extend(padding)
        vecs_np = np.array(vecs, dtype=np.float32)
        return torch.from_numpy(vecs_np), torch.tensor(
...
class BiLSTMClassifier(nn.Module):
    def __init__(self, hidden_dim=256, output_dim=2):
        ...
        self.lstm = nn.LSTM(
            300, 256, num_layers=2, dropout=0.5, bidirectional=True, batch_first=True
        )
