...
training_dataframe["label"] = label_encoder.fit_transform(training_dataframe["subtask_a"])

# Split the data into Train and Test
X_train, X_validation, y_train, y_validation = train_test_split(
    training_dataframe["tweet"], training_dataframe["label"], test_size=0.2, stratify=training_dataframe["label"], random_state=42
)
...
y_test_labels_dataframe = y_test_labels_dataframe.sort_values("id").reset_index(drop=True)
...
print("Classification Report:\n", classification_report(y_test_true, y_test_predict, target_names=label_encoder.classes_))
